		...
		switch (status) {
			case 400:
				...
				return (
					data.message ||
					data.error ||
					"Invalid request. Check your credentials."
				);
			case 401:
			...
			case 422:
				if (data.errors) {
					...
					Object.keys(data.errors).forEach((field) => {
						...
						fieldErrors[name] = Array.isArray(data.errors[field])
							? data.errors[field][0]
							: data.errors[field];
					});
					if (Object.keys(fieldErrors).length)
			...
			case 429:
				return (
					data.message || data.error || "Too many attempts. Wait a moment."
				);
			case 500:
			...
			default:
				return (
					data?.message || data?.error || `Error (${status}). Please try again.`
				);
		}
	};
